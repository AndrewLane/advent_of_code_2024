...
def count_stones_after_iterations(stones: List[int], iterations: int):
    if len(stones) > 1:
        return sum(
            [
    ...
    return count_stones_after_iterations(transformed_stones, iterations - 1)


def run_process(input):
    stones = [int(stone) for stone in input.split(" ")]
    return count_stones_after_iterations(stones, 25)
